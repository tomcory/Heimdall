...
class LibraryScannerViewModel @Inject constructor(
    ...
    val lastUpdatedInitial = repository.preferences.initialValues.libraryLastUpdatedInitial
    ...
    val lastUpdated = repository.preferences.libraryLastUpdated
    ...
    fun onScan(onShowSnackbar: (String) -> Unit) {
        viewModelScope.launch {
            ...
            if (scanResult) {
                repository.preferences.setLibraryLastUpdated(System.currentTimeMillis())
    ...
    private suspend fun scanAllApps(context: Context) : Boolean {
        ...
        filtered.forEach {
            ...
            // scan the third-party libraries of the app (also persists them)
            try {
