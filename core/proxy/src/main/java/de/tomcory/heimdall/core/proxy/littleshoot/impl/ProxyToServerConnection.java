...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
    ...
    /**
     * Disables SNI when initializing connection flow in {@link #initializeConnectionFlow(boolean)}. This value is set to true
    ...
    void write(Object msg) {
        ...
        if (is(DISCONNECTED) && msg instanceof HttpRequest) {
            ...
            connectAndWrite((HttpRequest) msg,true);
        } else {
            if (isConnecting()) {
    ...
    protected void exceptionCaught(Throwable cause) {
        try {
            if (cause instanceof IOException) {
                ...
                Timber.d("SSL Handshake Failed , disabling MITM for Host");
            }else {
     ...
     */
    private void connectAndWrite(HttpRequest initialRequest,boolean isMitm) {
        ...
        initializeConnectionFlow(!isMitm);
     ...
     * handling CONNECTs.
     */
    private void initializeConnectionFlow(boolean bypassMitm) {
    ...
    protected boolean connectionFailed(Throwable cause)
        ...
        if (!disableSni && cause instanceof SSLProtocolException) {
            ...
            if (cause.getMessage() != null && cause.getMessage().contains("unrecognized_name")) {
                ...
                connectAndWrite(initialRequest,false);

                return true;
            }
        }
        if(cause instanceof SSLHandshakeException || cause instanceof SSLProtocolException){
            MitmManager mitmManager = proxyServer.getMitmManager();
            HostAndPort parsedHostAndPort = HostAndPort.fromString(serverHostAndPort);
            ((SelectiveMitmManager)mitmManager).addWhiteListed(parsedHostAndPort.getHost()+parsedHostAndPort.getPort());
            resetConnectionForRetry();
            clientConnection.setMitming(false);
            connectAndWrite(initialRequest,false);
        ...
        if (chainedProxy != null) {
            ...
            connectAndWrite(initialRequest,true);
            return true;
        }
